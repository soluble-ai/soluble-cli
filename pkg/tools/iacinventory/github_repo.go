...
func (g *GithubRepo) SetTerraformDirs(d []string) {
	g.TerraformDirs = d
}

func (g *GithubRepo) SetCloudformationDirs(d []string) {
	g.CloudformationDirs = d
}

func (g *GithubRepo) SetDockerfileDirs(d []string) {
	g.DockerfileDirs = d
}

func (g *GithubRepo) SetK8sManifestDirs(d []string) {
	g.K8sManifestDirs = d
}

func (g *GithubRepo) SetCISystems(ciSys []string) {
	g.CISystems = ciSys
}
...
func (g *GithubRepo) scanTarball(dir, tarball string) error {
	...
	if err := scanRepo(g, dir); err != nil {
		return fmt.Errorf("could not scan tarball: %w", err)
	}
