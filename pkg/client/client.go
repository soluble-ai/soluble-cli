...
type Interface interface {
	...
	XCPPost(orgID string, module string, files []string, values map[string]string, options ...Option) (*resty.Response, error)
...
func (c *Client) XCPPost(orgID string, module string, files []string, values map[string]string, options ...Option) (*resty.Response, error) {
	if module == "" {
	...
	resp, err := req.Post(fmt.Sprintf("/api/v1/xcp/%s/data", module))
	return resp, err
}
